...
# T2MI (DVB-T2 Modulator Interface decapsulation)
SRCS-T2MI = \
...
# Vue UI — local build (vue_build) OR pcloud-cache fetch (vue_cache).
# Either path populates src/webui/static-vue/dist/ before bundling.
ALL-$(CONFIG_VUE_BUILD)   += vue_build
ALL-$(CONFIG_VUE_CACHE)   += vue_cache_get

# File bundles. CONFIG_VUE_BUILD and CONFIG_VUE_CACHE both cause the
# dist to be bundled; the rule for which produces it lives further
# down (near make_webui).
SRCS-${CONFIG_BUNDLE}     += bundle.c
BUNDLES-yes               += src/webui/static
BUNDLES-$(CONFIG_VUE_BUILD) += src/webui/static-vue/dist
BUNDLES-$(CONFIG_VUE_CACHE) += src/webui/static-vue/dist
...
# Vue UI cache fetch — pulls pre-built dist from pcloud. Used by
# downstream packagers that don't have a Node toolchain (or by
# anyone who wants to skip the ~2-3 min `npm ci` cost on every
# clean build). See Makefile.webui-vue for the targets.
ifeq ($(CONFIG_VUE_CACHE),yes)
.PHONY: vue_cache_get
vue_cache_get:
	$(MAKE) -f Makefile.webui-vue libcacheget

# Same bundling order constraint as vue_build above: dist must
# exist before MKBUNDLE walks it.
$(BUILDDIR)/bundle.c: vue_cache_get
endif

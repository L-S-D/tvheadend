  ...
  ( dvb_network_t *ln, dvb_mux_conf_t *dmc )
{
  ...
  LIST_FOREACH(mm, &ln->mn_muxes, mm_network_link) {
    ...
    /* Must match: FE type, frequency, polarisation, PID, IP, port */
    if (lm->lm_tuning.dmc_fe_type != dmc->dmc_fe_type) continue;
    if (deltaU32(lm->lm_tuning.dmc_fe_freq, dmc->dmc_fe_freq) > 2000) continue;
    if (lm->lm_tuning.u.dmc_fe_qpsk.polarisation != dmc->u.dmc_fe_qpsk.polarisation) continue;
    if (lm->lm_tuning.dmc_fe_pid != dmc->dmc_fe_pid) continue;
  ...
  ( dvb_network_t *ln, dvb_mux_conf_t *dmc )
{
  ...
  LIST_FOREACH(mm, &ln->mn_muxes, mm_network_link) {
    ...
    /* Must match: FE type, frequency, polarisation, PID, ETI params */
    if (lm->lm_tuning.dmc_fe_type != dmc->dmc_fe_type) continue;
    if (deltaU32(lm->lm_tuning.dmc_fe_freq, dmc->dmc_fe_freq) > 2000) continue;
    if (lm->lm_tuning.u.dmc_fe_qpsk.polarisation != dmc->u.dmc_fe_qpsk.polarisation) continue;
    if (lm->lm_tuning.dmc_fe_pid != dmc->dmc_fe_pid) continue;
    if (lm->lm_tuning.dmc_dab_eti_padding != dmc->dmc_dab_eti_padding) continue;
    if (lm->lm_tuning.dmc_dab_eti_bit_offset != dmc->dmc_dab_eti_bit_offset) continue;
    if (lm->lm_tuning.dmc_dab_eti_inverted != dmc->dmc_dab_eti_inverted) continue;
  ...
  ( dvb_network_t *ln, dvb_mux_conf_t *dmc )
{
  ...
  LIST_FOREACH(mm, &ln->mn_muxes, mm_network_link) {
    ...
    /* Must match: FE type, frequency, polarisation, stream_id (ISI), IP, port */
    if (lm->lm_tuning.dmc_fe_type != dmc->dmc_fe_type) continue;
    if (deltaU32(lm->lm_tuning.dmc_fe_freq, dmc->dmc_fe_freq) > 2000) continue;
    if (lm->lm_tuning.u.dmc_fe_qpsk.polarisation != dmc->u.dmc_fe_qpsk.polarisation) continue;
    if (lm->lm_tuning.dmc_fe_stream_id != dmc->dmc_fe_stream_id) continue;
